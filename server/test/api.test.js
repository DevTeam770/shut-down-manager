...
describe('חבילת לינור: הודעות, צ׳קליסט פרטי, משמעויות+מסמך, נהלים', () => {
  ...
  it('משמעויות + מסמך מרוכז כשכולם מאשרים', async () => {
    ...
    await member.post(`/api/shutdowns/${sid}/respond`).send({ response: 'approved', impact_text: 'שירות הדוחות יורד' }).expect(200);
    let full = await manager.post(`/api/shutdowns/${sid}/respond`).send({ response: 'approved', impact_text: 'אין השפעה על הצוות שלי' }).expect(200);
